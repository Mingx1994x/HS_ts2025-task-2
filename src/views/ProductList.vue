...
<template>
  <div class="position-relative d-flex align-items-center justify-content-center product-list-hero"
      style="min-height: 482px;">
      <div class="position-absolute"
        style="top:0; bottom: 0; left: 0; right: 0; background-image: url(https://images.unsplash.com/photo-1507643179773-3e975d7ac515?auto=format&fit=crop&q=80&w=1350); background-position: center center; background-size: cover; opacity: 0.2;">
      </div>
        ...
        <div class="col-md-9">
          <div class="row g-4">
            <div 
              ...
              :key="product.id"
            >
              <div class="card product-item-card border-0 h-100 position-relative">
                <img :src="product.imageUrl"
                  class="card-img-top object-fit-cover rounded-top" :alt="product.title" style="height: 280px;">
                <a href="#" class="position-absolute product-wishlist-icon" style="right: 12px; top: 12px">
                  <i class="far fa-heart"></i>
                </a>
                <div class="card-body">
                  <h4 class="fs-6 mt-2">
                      <RouterLink :to="`/product-detail/${product.id}`">{{product.title}}</RouterLink>
                  </h4>
                  <div class="d-flex align-items-center">
                    <span class="text-primary fw-bold me-2">NT$ {{ product.price.toLocaleString() }}</span>
                    <small class="text-neutral text-decoration-line-through">NT$ {{ product.origin_price.toLocaleString() }}</small>
                  </div>
                </div>
              </div>
            </div>
